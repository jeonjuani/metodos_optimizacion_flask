...
def segunda_derivada(f_expr, x, h=1e-6):
    return (evaluar_funcion(f_expr, x + h) - 2 * evaluar_funcion(f_expr, x) + evaluar_funcion(f_expr, x - h)) / (h * h)
...
def _iterar(f_expr, x0, tol, max_iter):
    ...
    for i in range(1, max_iter + 1):
        ...
        fppx = segunda_derivada(f_expr, x)

        if abs(fppx) < 1e-12:
            raise ValueError(f"La segunda derivada es cero o cercana a cero en x = {x:.12f}. Imposible continuar.")
...
def ejecutar(f_expr, x0, tol=1e-6, max_iter=100):
    ...
    punto_optimo = iteraciones[-1]["x_next"] if iteraciones else x0

    a = min(x0, punto_optimo) - 1
    b = max(x0, punto_optimo) + 1
